#ifndef MATHIC_MATRIX_HPP_
#define MATHIC_MATRIX_HPP_
...
namespace mathic {
  namespace detail {
    ...
    struct matrix_builder<detail::indexes_tuple<RowIndexes...>, CC, T> : detail::data_holder<RowIndexes, vector<T, CC>>... {
      ...
      matrix_builder() : detail::data_holder<RowIndexes, row_type>()... {
      }
      template <typename... E>
      matrix_builder(vector<E, CC> const &... args) : detail::data_holder<RowIndexes, row_type>(args)... {
      }
      matrix_builder(std::initializer_list<std::initializer_list<value_type>> const &args) : detail::data_holder<RowIndexes, row_type>(row_type(*(args.begin() + RowIndexes)))... {
      }
      template <size_t N>
 ...
 */
  ...
  struct matrix : detail::matrix_builder<typename detail::index_builder<RC>::type, CC, T> {
    ...
    matrix(std::initializer_list<std::initializer_list<value_type>> const &args) : super_type(args) {
    }
    template <typename... E>
    matrix(E const &... args) : super_type(args...) {
    }
    template <typename U>
    this_type &
